...
def createJobScript(img_type, batch_num):
    datadir = '/gpfs/loomis/project/fas/david_moore/zl423/dlphysics/SparseConvNet/examples/nEXOSparse/'
    script = open('%s/job/%s_%d.sh' % ( datadir,img_type, batch_num) ,'w')
    script.write('#!/bin/bash \n')
    script.write('#SBATCH -t 24:00:00 \n')
    script.write('#SBATCH --mem=18000\n')
    script.write('date\n')
    script.write('source /gpfs/loomis/project/fas/david_moore/zl423/dlphysics/SparseConvNet/setup.sh\n')
    script.write('cd /gpfs/loomis/project/fas/david_moore/zl423/dlphysics/SparseConvNet/examples/nEXOSparse\n')
